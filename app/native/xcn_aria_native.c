...
int xcn_aria_encrypt_blocks(const unsigned char *key, int keyBits, const unsigned char *input, unsigned long length, unsigned char *output) {
  Byte rk[16*17];
  int rounds;
  unsigned long offset;

  if (key == NULL || input == NULL || output == NULL) return -1;
  if (keyBits != 128 && keyBits != 192 && keyBits != 256) return -2;
  if (length % 16 != 0) return -3;

  rounds = EncKeySetup(key, rk, keyBits);
  for (offset = 0; offset < length; offset += 16) {
    Crypt(input + offset, rounds, rk, output + offset);
  }
  return 0;
}

int xcn_aria_decrypt_blocks(const unsigned char *key, int keyBits, const unsigned char *input, unsigned long length, unsigned char *output) {
  Byte rk[16*17];
  int rounds;
  unsigned long offset;
  ...
  rounds = DecKeySetup(key, rk, keyBits);
  for (offset = 0; offset < length; offset += 16) {
    Crypt(input + offset, rounds, rk, output + offset);
  }
  return 0;
